...
```bash
# User-level (works across all repos)
git clone https://github.com/blader/continual-learning-scratchpad.git ~/.claude/skills/continual-learning-scratchpad
```
...
1. **Session start**: Agent reads `.claude/SCRATCHPAD.md` in the current repo. If it doesn't exist, it creates one.
...
3. **Over sessions**: The scratchpad compounds. Session 1 is normal. Session 3 the agent is catching things before you do. Session 5 it's a different tool.
...
Lives at `.claude/SCRATCHPAD.md` in each repo. One per repo. The agent designs the initial structure and adapts it to the project's domain.
